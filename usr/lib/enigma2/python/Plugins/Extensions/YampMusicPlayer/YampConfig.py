...
class YampConfigScreenV33(Screen, ConfigListScreen):
	...
	def __init__(self, session, parent, videoPreviewOnPar):
		...
		if getDesktop(0).size().width() > 1280:	WIDTH = 151
		else:	WIDTH = 130
		...
		if os.path.exists('/etc/image-version'):
			...
			lines = fm.read()
			self.isVTi = 'vti' in lines.lower()	
			fm.close()
#		self.isVTi=True	

		dummy1, self.txtRcPvrVideo, txtBouq = readRcButtonTexts(self.isVTi)
		...
		if os.path.exists("/var/lib/dpkg/status"): self.startTimer_conn=self.startTimer.timeout.connect(self.startActions)
		else:	self.startTimer.callback.append(self.startActions)
		self.updateTimer = eTimer()
		if os.path.exists("/var/lib/dpkg/status"): self.updateTimer_conn=self.updateTimer.timeout.connect(self.updateTimerActions)
		else:	self.updateTimer.callback.append(self.updateTimerActions)
	...
	def layoutFinished(self):
		dummy1, dummy2, txtBouq = readRcButtonTexts(self.isVTi)
		...
		skinPath = os.path.join(yampDir, 'skins/')
		if not os.path.isfile(skinPath + 'custom/Yamp.xml'): self.deleteSkinOption('custom')
		if not os.path.isfile(skinPath + 'fhdCustom/Yamp.xml'): self.deleteSkinOption('fhdCustom')
		if not os.path.isfile(skinPath + 'customDreamOS/Yamp.xml'): self.deleteSkinOption('customDreamOS')
		if not os.path.isfile(skinPath + 'fhdCustomDreamOS/Yamp.xml'): self.deleteSkinOption('fhdCustomDreamOS')
...
#		LOG('YampConfigScreen: deleteSkinOption: skin: %s' %(skin), 'all') 
	...
	def calcNewLCDvalues(self):
		...
		if lcdSize == 'custom':
			...
			if customCoverSize == -1 : #new  custom could not read cover size from file
				if self.callingKeyLeft: ConfigListScreen.keyRight(self)
				else: ConfigListScreen.keyLeft(self)
				return
			else:
			...
			try:
				...
				if lcdSizeChanged: # or self.previousLcdMode != lcdMode: 
					...
					if lcdSizeChanged:
						...
						else: textLen = lcdWidth - 2 * textMinLeft
	...
	def CalcCoverLeftTopValue(self, lcdSizeList, coverSize): #lcdSizeList: 0: Width 1: Height
		...
		else: posX = int(lcdSizeList[0])-coverSize-textMinLeft
		if alignVer == 'top':
			posY = textMinTop
		elif alignVer == 'center':
			posY = (int(lcdSizeList[1])-coverSize)/2
		else: posY = int(lcdSizeList[1])-coverSize-textMinTop
		if posX < 0: posX = 0
		if posY < 0: posY = 0
		return posX,posY
		...
		try:
			for element in self.renderer:
				if "eVideoWidget" in str(vars(element)):
					if show: element.instance.show()
					else: element.instance.hide()
		except Exception as e:
