...
from enigma import eListboxPythonMultiContent, eListbox, getDesktop, gFont, RT_VALIGN_TOP, RT_VALIGN_CENTER, RT_VALIGN_BOTTOM, RT_HALIGN_LEFT, RT_HALIGN_RIGHT, RT_HALIGN_CENTER
...
class YampDatabaseList(GUIComponent):
	...
	def buildEntry(self, item):
		...
		if item.nav: align1 = RT_HALIGN_CENTER | self.alignVertLine1
		else: align1 = RT_HALIGN_LEFT | self.alignVertLine1
			...
			try:
				for (attrib, value) in self.skinAttributes:
					if attrib == "itemFont0":
						...
						if value.lower() == 'top': self.alignVertLine1 = RT_VALIGN_TOP
						elif value.lower() == 'bottom': self.alignVertLine1 = RT_VALIGN_BOTTOM
						else: self.alignVertLine1 = RT_VALIGN_CENTER
					elif attrib == "itemVAlignLine2":
						if value.lower() == 'top': self.alignVertLine2 = RT_VALIGN_TOP
						elif value.lower() == 'bottom': self.alignVertLine2 = RT_VALIGN_BOTTOM
						else: self.alignVertLine2 = RT_VALIGN_CENTER
	...
	def setLines(self, lines):
		...
		if self.parItemSizeLine1exist: self.myItemHeightF = self.line1sizeY
		else: 
			self.myItemHeightF = self.font0h * 1.1 
			self.line1sizeY = int(self.font0h * 1.05)
		if self.lines == 2:
			if self.parHeightLine2exist: self.myItemHeightF += self.line2sizeY
			else: self.myItemHeightF += self.font1h * 1.1
	...
	def getColor(self, colString):
		try: return parseColor(colString).argb()
		except:	return 0xFF0000
