...
class YampLyricsScreenV33(Screen, HelpableScreen):
	def __init__(self, session, parent, videoPreviewOnPar):
		...
		self.triggerTimer.callback.append(self.triggerStartActions)
...
#1..4 are only the 4 methods; the actual search sequence is defined in the config
	...
	def searchAZlyrics(self):
		...
		if config.plugins.yampmusicplayer.prioLyrics1.value == 'lyricsAZ':
			prio = 1
		elif config.plugins.yampmusicplayer.prioLyrics2.value == 'lyricsAZ':
			prio = 2
		elif config.plugins.yampmusicplayer.prioLyrics3.value == 'lyricsAZ':
			prio = 3
		elif config.plugins.yampmusicplayer.prioLyrics4.value == 'lyricsAZ':
			prio = 4
		else:
			return
...
#---------- hier war genius
	...
	def searchChartlyrics(self):
		...
		if config.plugins.yampmusicplayer.prioLyrics1.value == 'lyricsChart':
			prio = 1
		elif config.plugins.yampmusicplayer.prioLyrics2.value == 'lyricsChart':
			prio = 2
		elif config.plugins.yampmusicplayer.prioLyrics3.value == 'lyricsChart':
			prio = 3
		elif config.plugins.yampmusicplayer.prioLyrics4.value == 'lyricsChart':
			prio = 4
		else:
			return
...
#--------------------- edit karaoke timestamp
	...
	def buildNewLyrics(self):
		...
		try:
			if self.lyricsAutoSaveTimer.isActive():
	...
	def showMenu(self):						#Build Menu Entries
		...
		try:
			if self.lyricsEditMode:
				menu.append(((_('deactivate edit mode (scroll over empty lines)')), 'editmode'))
			else:
				menu.append(((_('activate edit mode (empty lines also accessible)')), 'editmode'))
...
#--------------- help functions
	...
	def setTextYellowKey(self):
		if self.autoMoveOn:
			self["key_yellow"].setText(_("Auto move Off"))
		else:
			self["key_yellow"].setText(_("Auto move On"))

	def setTextBlueKey(self):
		if self.allowTimeEdit:
			self["key_blue"].setText(_("Disallow time edit"))
		else:
			self["key_blue"].setText(_("Allow time edit"))
	...
	def showHideVideoPreview(self):
		if self.pigElement is not None:
			if self.videoPreviewOn is True:
