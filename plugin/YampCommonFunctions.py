...
def readID3Infos(path):
	...
	if path.lower().endswith(".wma"):
		...
		try:
			...
			try:
				...
				LOG('readID3Infos: getFlacBitrate: EXCEPT', 'err')
		except:
			audio = None
		...
		try:
			...
			audio = None
			try:
				audio = mutagen.File(path, easy=True)		#new method
			except:
				audio = None
	...
	if audio:
		try:
			...
			LOG('readID3Infos: get title: EXCEPT', 'err')
		try:
			album = getEncodedString(audio.get('album', ['n/a'])[0])
		except:
			LOG('readID3Infos: title: %s: get album: EXCEPT' % (title), 'err')
		try:
			artist = getEncodedString(audio.get('artist', ['n/a'])[0])
		except:
			LOG('readID3Infos: title: %s album: %s: get artist: EXCEPT' % (title, album), 'err')
		try:
			genre = getEncodedString(audio.get('genre', ['n/a'])[0])
		except:
			LOG('readID3Infos: title: %s artist: %s: get genre: EXCEPT' % (title, artist), 'err')
		try:
			date = getEncodedString(audio.get('date', ['n/a'])[0])
		except:
			LOG('readID3Infos: title: %s artist: %s: get date: EXCEPT' % (title, artist), 'err')
		try:
			tracknr = searchTrackAndDiscNumber(audio)
		except Exception as e:
			LOG('readID3Infos: filename: %s tracknr: EXCEPT %s' % (filename, str(e)), 'err')
		if bitrate == 0.0:
			try:
		...
		try:
			...
			length = str(datetime_timedelta(seconds=len))
			if len < 3600:
				length = length[-5:]
		except Exception as e:
			length = "n/a"
			LOG('readID3Infos: filename: %s length: EXCEPT %s' % (filename, str(e)), 'err')
	else:
		title = filename
	if bitrate > 0.0:
...
def getPage(url, params=None, headers=None, cookies=None, timeout=None):
	...
	return deferToThread(requests.get, url, params=params, headers=headers, cookies=cookies, timeout=timeout or 30.05)
