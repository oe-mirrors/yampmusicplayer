...
def readID3Infos(path):
	...
	if path.lower().endswith(".wma"):
		...
		try:
			...
			try:
				...
				if duration > 0:
					bitrate = filesize / duration * 8.0 / 1000.0
			except OSError:
				LOG('YampCommonFunctions: readID3Infos: getFlacBitrate: EXCEPT', 'err')
		except Exception:
			audio = None
	...
	if audio:
		try:
			...
			LOG('YampCommonFunctions: readID3Infos: get title: EXCEPT', 'err')
		try:
			album = audio.get('album', ['n/a'])[0]
		except Exception:
			LOG('YampCommonFunctions: readID3Infos: title: %s: get album: EXCEPT' % (title), 'err')
		try:
			artist = audio.get('artist', ['n/a'])[0]
		except Exception:
			LOG('YampCommonFunctions: readID3Infos: title: %s album: %s: get artist: EXCEPT' % (title, album), 'err')
		try:
			genre = audio.get('genre', ['n/a'])[0]
		except Exception:
			LOG('YampCommonFunctions: readID3Infos: title: %s artist: %s: get genre: EXCEPT' % (title, artist), 'err')
		try:
			albumartist = audio.get('albumartist', [''])[0]
		except Exception as e:
			albumartist = ''
			LOG('YampCommonFunctions: readID3Infos: title: %s album: %s: get albumartist: EXCEPT:' + str(e) % (title, album), 'err')
		if albumartist == '':
			albumartist = artist
		try:
			date = audio.get('date', ['n/a'])[0]
		except Exception:
			LOG('YampCommonFunctions: readID3Infos: title: %s artist: %s: get date: EXCEPT' % (title, artist), 'err')
		try:
			tracknr = searchTrackAndDiscNumber(audio)
		except Exception as e:
			LOG('YampCommonFunctions: readID3Infos: filename: %s tracknr: EXCEPT %s' % (filename, str(e)), 'err')
		if bitrate == 0:
			try:
				bitrate = audio.info.bitrate / 1000.0
			except Exception:
				pass
		try:
			alen = int(audio.info.length)
			length = str(timedelta(seconds=alen))
			if alen < 3600:
				length = length[-5:]
		except Exception as e:
			length = "n/a"
			LOG('YampCommonFunctions: readID3Infos: filename: %s length: EXCEPT %s' % (filename, str(e)), 'err')
	else:
		title = filename
	if bitrate > 0.0:
		strBitrate = str(round(bitrate, 1)) + ' kBit/s'
...
def getUrlData(url, filename=None, params=None, headers=None, timeout=(3.05, 6), callback=None, fail=None):
	headers = headers or {}
	headers["user-agent"] = "Mozilla/5.0 (Linux; Android 14; SM-S918W Build/UP1A.231005.007; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/122.0.6261.119 Mobile Safari/537.36/122.0.6261.119"
	errMsg = ""
	try:
		response = get(url, params=params, headers=headers, timeout=timeout, stream=True)
		response.raise_for_status()
	except exceptions.RequestException as e:
		errMsg = str(e)
		LOG('YampCommonFunctions: getUrlData_get: EXCEPT %s' % errMsg, "err")
		response = None
	if response and filename:
		try:
			...
			errMsg = str(e)
			LOG('YampCommonFunctions: getUrlData_writ: EXCEPT %s' % errMsg, "err")
	if errMsg:
		if fail:
			fail(str(errMsg))
		return str(errMsg)
