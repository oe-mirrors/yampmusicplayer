...
def searchLyricsfromFiles(songFilename, foundPrio):
	...
	if config.plugins.yampmusicplayer.prioLyrics1.value == 'lyricsFile': prio = 1
	elif config.plugins.yampmusicplayer.prioLyrics2.value == 'lyricsFile': prio = 2
	elif config.plugins.yampmusicplayer.prioLyrics3.value == 'lyricsFile': prio = 3
	elif config.plugins.yampmusicplayer.prioLyrics4.value == 'lyricsFile': prio = 4
	else: return '','',foundPrio
...
def getLyricsID3(songFilename,foundPrio):
	if config.plugins.yampmusicplayer.prioLyrics1.value == 'lyricsID3': prio = 1
	elif config.plugins.yampmusicplayer.prioLyrics2.value == 'lyricsID3': prio = 2
	elif config.plugins.yampmusicplayer.prioLyrics3.value == 'lyricsID3': prio = 3
	elif config.plugins.yampmusicplayer.prioLyrics4.value == 'lyricsID3': prio = 4
	else: return '',LYRICSS_NO, foundPrio
		...
		try:
			for key in f.keys():
				if  key.endswith('lyr'):
					...
					if songFilename.lower().endswith(".mp4"):	pixNumLyrics = LYRICSS_MP4
					else: pixNumLyrics = LYRICSS_M4A
					foundPrio = prio
		except Exception as e:
	...
	try:
		...
		for idx in range(len(tiStampMsec90)):
			if tiStampMsec90[idx] > 0:
				...
				if tiStampMsec90[idx] < 0: tiStampMsec90[idx] = 90
